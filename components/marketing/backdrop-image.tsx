...
// telling you exactly which file to drop into public/assets/landing/backdrops/. As
...
export function BackdropImage({
  ...
  if (errored) {
    ...
    return (
      <div className={cn("absolute inset-0 bg-[var(--strap-surface-raised)]", className)}>
        <div className="absolute left-1/2 top-1/2 max-w-[16rem] -translate-x-1/2 -translate-y-1/2 px-6 text-center">
          ...
          <div
            className="mt-2 rounded-md bg-[var(--strap-surface)] px-2.5 py-1.5 text-[11px] leading-relaxed text-[var(--strap-text-tertiary)]"
            style={{ fontFamily: "var(--font-mono), monospace" }}
          >
            public/assets/landing/backdrops/
            <br />
            {fileName}
          </div>
          {hint ? (
            <div className="mt-2 text-[11px] text-[var(--strap-text-tertiary)]">{hint}</div>
          ) : null}
        </div>
      </div>
    );
