...
type StrapContextValue = {
  ...
  // background. Returns false only when it can't even start (no active Strap).
  fileProposalEdit: (sectionId: string, content: string) => boolean;
  reorderSections: (sectionIds: string[]) => void;
  addSection: (name: string, starter?: string) => StrapSection;
  addSectionAfter: (
  ...
  setVersionControlConfig: (
    patch: Partial<StrapSettings["versionControl"]>,
  ) => void;
  ...
  importSections: (sections: StrapSection[]) => Promise<void>;
  deleteAccount: () => Promise<void>;
  updateOnboarding: (patch: Partial<StrapState["onboarding"]>) => void;
  resetOnboarding: () => void;
  claimOnboardingPreview: (sections: StrapSection[]) => Promise<void>;
...
function nextMutationTick(state: StrapState) {
  return {
    ...state,
...
function mergeExternalState(
  ...
  let sections: StrapState["sections"];
  ...
  return {
    ...current,
    // Company / switcher context always reflects the server (which Strap is
...
function stabilizeMergedState(
  ...
  const scalarsEqual = (Object.keys(stabilized) as Array<keyof StrapState>).every(
    (key) => {
...
function cloneSection(section: StrapSection): StrapSection {
...
function getProposalBeforeText(
  section: StrapSection | undefined,
  _proposal: Proposal,
) {
...
function applyProposalToSection(section: StrapSection, proposal: Proposal) {
...
function bumpSectionRevisionMap(
  revisions: StrapState["sectionRevisions"],
  sectionId: string,
) {
  return {
    ...revisions,
...
export function StrapProvider({
  ...
  // the server yet - see mergeExternalState. Cleared wholesale on Strap switch.
  ...
  // successful save announces itself; other tabs on the same Strap resync
  ...
  const persistState = useCallback(
    async (nextState: StrapState, keepalive = false) => {
  ...
  const flushPendingState = useCallback(
    async (snapshot: StrapState, keepalive = false) => {
  ...
  function schedulePersist(snapshot: StrapState) {
  ...
  function commitState(updater: (current: StrapState) => StrapState) {
    if (latestStateRef.current.creedType === "company") {
  ...
  async function runCompanySectionCreate(section: StrapSection, afterSectionId?: string) {
    const creedId = latestStateRef.current.creedId;
    if (!creedId) return;
  ...
  const syncFromServer = useCallback(async () => {
    ...
    const payload = (await response.json()) as { state?: StrapState };
    if (!payload.state) {
      return;
    }

    setState((current) => {
      // Ignore a response for a different Strap (a background sync that raced a
      // switch). Cross-Strap transitions go through switchCreed's wholesale
      // replace; merging across Straps would pair a mismatched creedType with
  ...
  // Switch the active Strap instantly, client-side. We deliberately do NOT use
  ...
  // anyway. Instead we set the active-Strap cookie, fetch the target Strap's
  ...
  const switchCreed = useCallback(
    async (creedId: string): Promise<{ ok: boolean; error?: string }> => {
      ...
      const activate = await fetch("/api/app/straps/activate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    ...
    // follow the active Strap: company Straps poll fast for near-live updates,
    // personal Straps stay lazy. Reads creedType fresh each tick so switching
    // Straps adapts without restarting the effect.
  ...
  function updateRichTextSection(sectionId: string, content: string) {
    ...
    // save that logs a phantom "edited" activity row on a freshly loaded Strap.
  ...
  function reorderSections(sectionIds: string[]) {
    commitState((current) => {
      ...
      const reordered = sectionIds
        .map((id) => map.get(id))
        .filter((section): section is StrapSection => Boolean(section));
  ...
  // Archiving the whole Strap archives every live section at once and drops in a
  ...
  function archiveCreed() {
    ...
    commitState((current) => {
      ...
      const placeholder: StrapSection = {
        id: uniqueLocalId("section"),
        kind: "rich-text",
        template: "freeform",
        name: "New section",
  ...
  function clearSections() {
    ...
    const defaultSections = initialStrapState.sections.map((section) => ({
      ...section,
      lastEditedBy: "You",
  ...
  // Company mode: proposals are reviewed through the per-Strap API (the personal
  ...
  // from the response alone instead of refetching the whole Strap state; the
  ...
  function updateOnboarding(patch: Partial<StrapState["onboarding"]>) {
    setState((current) => ({
      ...current,
      onboarding: {
        ...current.onboarding,
  ...
  async function claimOnboardingPreview(sections: StrapSection[]) {
    const nextState = nextMutationTick({
      ...state,
      lastSavedAt: Date.now(),
      sections,
  ...
  async function importSections(sections: StrapSection[]) {
  ...
  function exportMarkdown() {
    return buildVisibleStrapMarkdown(state.sections);
  }
  ...
  type StrapActions = typeof actionsImpl;
  const actionsImplRef = useRef<StrapActions>(actionsImpl);
  ...
  const stableActions = useMemo(() => {
    ...
    for (const key of Object.keys(actionsImplRef.current)) {
      proxies[key] = (...args: unknown[]) =>
        (
          actionsImplRef.current[key as keyof StrapActions] as (
            ...a: unknown[]
          ) => unknown
        )(...args);
    }
    return proxies as unknown as StrapActions;
  ...
  return (
    <StrapContext.Provider value={contextValue}>
      {children}
    </StrapContext.Provider>
  );
}

export function useStrap() {
  const context = useContext(StrapContext);
...
/** @deprecated Use StrapProvider. Retained for component-import compatibility. */
export const CreedProvider = StrapProvider;
/** @deprecated Use useStrap. Retained for component-import compatibility. */
export const useCreed = useStrap;
