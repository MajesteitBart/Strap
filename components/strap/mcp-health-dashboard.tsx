...
// MCP health dashboard rendered under the Strap MCP card on /connections.
// Fetches an aggregated summary from /api/app/mcp/health (off the hot
// loadStrapState path) and renders it with recharts. Three filters drive the
...
} from "@/components/strap/mcp-health-preload";
...
export function McpHealthDashboard() {
  ...
  // Scope the client cache to the active Strap so switching Straps never shows
  // the previous one's dashboard. Empty string = personal/active Strap.
  ...
  useEffect(() => {
    ...
    // Re-fetch on range change, Strap switch, and whenever a new agent shows up.
    ...
    for (const section of filteredSummary.sections) {
      ...
      if (existing) {
        ...
        buckets.set(key, {
          ...
          color: current ? current.color : "var(--strap-text-tertiary)",
          count,
        });
      }
    }
  ...
  return (
    <div ref={rootRef} className="mt-12 scroll-mt-8 md:scroll-mt-10">
      ...
      {isEmpty ? (
        <div className="mt-5 rounded-xl border border-dashed border-[var(--strap-border)] bg-[var(--strap-surface)] px-6 py-12 text-center">
          <IntegrationGlyph
            kind="mcp"
            framed={false}
            className="mx-auto h-10 w-10 opacity-70"
          />
          <div className="mt-4 text-[15px] font-medium text-[var(--strap-text-primary)]">
            No MCP activity yet
          </div>
          <p className="mx-auto mt-2 max-w-sm text-[13px] leading-6 text-[var(--strap-text-secondary)]">
        ...
        <>
          ...
          <div className="mt-4 rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] p-5">
            ...
            {summary && chart.total > 0 ? (
              ...
              <div className="flex h-[240px] items-center justify-center text-[13px] text-[var(--strap-text-tertiary)]">
                {loading
                  ? "Loading…"
                  : metric === "all"
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="flex min-w-0 flex-col rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] p-5">
              <div className="text-[13px] font-medium text-[var(--strap-text-secondary)]">
                Proposal outcomes
              </div>
              {summary && outcomeTotal > 0 ? (
                ...
                <div className="flex min-h-[180px] flex-1 items-center justify-center text-[13px] text-[var(--strap-text-tertiary)]">
                  {loading ? "Loading…" : "No proposals in this range yet."}
                </div>
              )}
            </div>

            <div className="min-w-0 rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] p-5">
              <div className="text-[13px] font-medium text-[var(--strap-text-secondary)]">
                Section coverage
              </div>
              {sections.length > 0 ? (
                <div className="mt-2 flex items-center gap-6">
                  ...
                  >
                    ...
                    <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
                      <span className="text-[22px] font-medium leading-none tracking-[-0.03em] text-[var(--strap-text-primary)]">
                        {activeSection !== null && sections[activeSection]
                          ? sections[activeSection].count
                          : sectionTotal}
                      </span>
                      <span className="mt-1 max-w-full truncate text-[12px] text-[var(--strap-text-tertiary)]">
                        {activeSection !== null && sections[activeSection]
                  ...
                  <div
                    className="max-h-[200px] min-w-0 flex-1 space-y-1 overflow-y-auto pr-1 strap-scrollbar"
                    onMouseLeave={() => setActiveSection(null)}
                  >
                    {sections.map((section, index) => (
                      <div
                      ...
                      >
                        ...
                        />
                        <span className="hidden truncate text-[var(--strap-text-secondary)] sm:block">
                          {section.sectionName}
                        </span>
                        <span className="ml-auto shrink-0 font-mono text-[var(--strap-text-tertiary)]">
                          {section.count}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex h-[180px] items-center justify-center text-[13px] text-[var(--strap-text-tertiary)]">
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            "inline-flex h-8 max-w-full shrink-0 items-center gap-2 rounded-md px-3 text-[14px] text-[var(--strap-text-primary)] transition-colors duration-150 hover:bg-[var(--strap-surface-raised)] disabled:pointer-events-none disabled:opacity-60",
            variant === "outline"
              ? "border border-[var(--strap-border)] bg-[var(--strap-surface)]"
              : "-ml-1 text-[14px] font-medium text-[var(--strap-text-secondary)]",
            triggerClassName,
          )}
        >
          {triggerIcon ? (
          ...
          <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[var(--strap-text-secondary)]" />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
...
function StatTile({
  ...
  return (
    <div className="rounded-lg border border-[var(--strap-border)] bg-[var(--strap-surface)] p-4">
      <div className="text-[12px] font-medium text-[var(--strap-text-secondary)]">
        {label}
      </div>
      <div className="mt-2 text-[28px] font-medium leading-none tracking-[-0.04em] text-[var(--strap-text-primary)]">
        {value ?? <span className="text-[var(--strap-text-tertiary)]">-</span>}
      </div>
      {sub ? (
        <div className="mt-2 text-[12px] text-[var(--strap-text-tertiary)]">
          {sub}
        </div>
      ) : null}
    </div>
