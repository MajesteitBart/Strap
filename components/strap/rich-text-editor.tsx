      ...
      attributes: {
        class:
          density === "continuation"
            ? "continuation-editor min-h-[56px] pb-0 text-[var(--strap-text-primary)]"
            : "min-h-[56px] pb-2 text-[var(--strap-text-primary)]",
      },
            ...
            <AnimatePresence>
              {editor && selectionToolbar && !readOnly ? (
                <motion.div
                  ...
                  className={cn(
                    "fixed z-50 flex -translate-x-1/2 items-center gap-0.5 rounded-lg border border-[var(--strap-border)] bg-[var(--strap-surface)] p-1 text-[var(--strap-text-primary)] shadow-[0_6px_20px_rgba(28,28,26,0.10)]",
            ...
            <AnimatePresence>
              {slashState && slashState.items.length > 0 ? (
                <motion.div
                  ...
                  className="fixed z-50 w-[220px] overflow-hidden rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] p-1 shadow-[0_8px_24px_rgba(28,28,26,0.08)]"
                  style={{
                    ...editorThemeStyle,
                    left: slashState.x,
                  ...
                  {slashState.items.map((command, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="editor-command-item flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-[13px] text-[var(--strap-text-primary)] transition-colors duration-100"
                      ...
                      >
                        <Icon className="h-3.5 w-3.5 shrink-0 text-[var(--strap-text-tertiary)]" />
                        <span className="flex-1 truncate font-medium">
                          {command.title}
                        </span>
                        {isActive ? (
                          <span className="text-[11px] text-[var(--strap-text-tertiary)]">
                            {command.description}
                          </span>
                        ) : null}
                      </button>
...
function ToolbarButton({
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        "flex h-7 w-7 items-center justify-center rounded-md text-[var(--strap-text-secondary)] transition-colors duration-100 hover:bg-[var(--strap-surface-raised)] hover:text-[var(--strap-text-primary)] disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:bg-transparent disabled:hover:text-[var(--strap-text-secondary)]",
        active &&
...
function ToolbarDivider() {
  return (
    <span aria-hidden className="mx-0.5 h-4 w-px bg-[var(--strap-border)]" />
  );
}
