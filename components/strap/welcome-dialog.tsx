...
// in the app (see app/(strap-app)/layout.tsx, which computes `show`). Slides:
...
// The company-only slide, shown second (right after "Your Strap is live").
...
function SlideTitleIcon({ slide }: { slide: Slide }) {
  ...
  return (
    <motion.div
      aria-hidden
      className="flex shrink-0 items-center text-[var(--strap-text-primary)]"
      initial={{
        scale: reduce ? 1 : 0.4,
        rotate: reduce ? 0 : -12,
        opacity: 0,
...
function MediaFrame({
  ...
  return (
    <motion.div variants={lineVariants} className="px-7 pt-4 pb-0">
      ...
      <div className="relative aspect-video max-h-[min(45dvh,394px)] w-full overflow-hidden rounded-[var(--radius-lg)] bg-[var(--strap-surface-raised)] ring-1 ring-inset ring-[var(--strap-border)]">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle, var(--strap-border-strong) 1px, transparent 1px)",
        ...
        <div className="absolute inset-0 grid place-items-center">
          <div
            className="grid h-14 w-14 place-items-center rounded-[var(--radius-md)] bg-[var(--strap-surface)] ring-1 ring-[var(--strap-border)] transition-colors duration-[400ms] dark:ring-[var(--strap-border-strong)]"
            style={{
        ...
        {showRedline ? (
          <div className="absolute bottom-3 left-3 rounded-[var(--radius-sm)] bg-[var(--strap-surface)]/70 px-2 py-1 font-mono text-[11px] leading-[1.5] ring-1 ring-[var(--strap-border)] backdrop-blur-[2px] dark:ring-[var(--strap-border-strong)]">
            <div className="text-[var(--strap-text-tertiary)]">
              16:9 · placeholder
            </div>
            <div className="text-[var(--strap-text-secondary)]">
...
export function WelcomeDialog({
  ...
  const accentVars =
    variant === "company"
      ...
      : "[--tour-accent:var(--strap-accent)] [--tour-accent-hover:var(--strap-accent-hover)]";
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        className={cn(
          "strap-scrollbar flex flex-col gap-0 overflow-x-hidden overflow-y-auto p-0",
          "w-[calc(100vw-1.5rem)] max-w-[700px] sm:w-[calc(100vw-2rem)]",
          "max-h-[calc(100dvh-2rem)]",
          "bg-[var(--strap-surface)]",
          accentVars,
        )}
        onKeyDown={(e) => {
        ...
        <div className="grid overflow-x-hidden">
          <AnimatePresence initial={false} custom={dir}>
            ...
            >
              ...
              <motion.p
                variants={lineVariants}
                className="mt-2.5 min-h-[3.2rem] max-w-[52ch] px-7 text-[15px] leading-[1.7] text-[var(--strap-text-secondary)]"
              >
                {slide.body}
              </motion.p>
      ...
      {slides.map((s, i) => {
        ...
        return (
          <button
            ...
            onClick={() => onJump(i)}
            className={cn(
              "h-[5px] rounded-[3px] outline-none transition-all duration-300 focus-visible:ring-2 focus-visible:ring-[var(--tour-accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--strap-surface)]",
              current
                ? "w-5 bg-[var(--tour-accent)]"
                : done
                  ? "w-2.5 bg-[var(--tour-accent)]/45"
                  : "w-2.5 bg-[var(--strap-border-strong)]",
            )}
          />
        );
      })}
