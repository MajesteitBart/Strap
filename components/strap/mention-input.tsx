...
import { accentColorMap, type AccentKey } from "@/lib/strap-data";
  ...
  function MentionInput({ sections, placeholder, onChange, onKeyDown }, ref) {
    ...
    const insertMention = useCallback(
      (section: MentionSection) => {
        ...
        chip.className = "strap-inline-tag strap-mention-chip";
        const color = accentColorMap[section.accent] ?? "var(--strap-accent)";
    ...
    return (
      <div className="relative flex-1">
        {empty ? (
          <span className="pointer-events-none absolute left-0 top-1/2 -translate-y-1/2 text-[15px] text-[var(--strap-text-tertiary)]">
            {placeholder}
          </span>
        ) : null}
        <div
          ...
          className="min-h-[52px] w-full whitespace-pre-wrap break-words py-[15px] text-[15px] leading-[1.4] text-[var(--strap-text-primary)] outline-none"
        />
