...
const brandmark = "/assets/brand/strap-logo.svg";
...
// The Strap brandmark rendered in brand blue for the in-app agent's
// identity on proposal cards and activity rows. The logo SVG is used as a mask
// over a solid accent fill, so the shape reads as exactly --strap-accent in
// both themes (a plain <img> can't be recoloured to an exact hue).
export function StrapAgentGlyph({ className }: { className?: string }) {
  return (
    <span
      role="img"
      aria-label="Strap"
      ...
      style={{
        backgroundColor: "var(--strap-accent)",
...
/** @deprecated Use StrapWordmark. Retained until the staged product-path migration. */
export const CreedWordmark = StrapWordmark;
/** @deprecated Use StrapMark. Retained until the staged product-path migration. */
export const CreedMark = StrapMark;
/** @deprecated Use StrapAgentGlyph. Retained until the staged product-path migration. */
export const CreedAgentGlyph = StrapAgentGlyph;
...
export function IntegrationGlyph({
  ...
  return (
    <div
      className={cn(
        framed
          ? "flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] text-[var(--strap-text-primary)]"
          : "flex items-center justify-center text-[var(--strap-text-primary)]",
        className
      )}
    >
      {asset ? (
        <div className={cn("relative h-5 w-5", !framed && "h-9 w-9", assetClassName)}>
          <Image
            ...
            className={cn(
              ...
              MONOCHROME_AGENTS.has(kind) && "strap-invert-on-dark",
