...
function inlineElement(token: string, key: number): ReactNode {
  if (token.startsWith("`")) {
    return (
      <code key={key} className="rounded-[5px] bg-[var(--strap-surface-raised)] px-1 py-0.5 font-mono text-[0.85em]">
        {token.slice(1, -1)}
      </code>
    );
  }
    ...
    <div className={`break-words ${className ?? ""}`}>
      {blocks.map((block, index) => {
        if (block.kind === "heading") {
          ...
          return (
            <div key={index} className={`mt-2 mb-1 font-semibold text-[var(--strap-text-primary)] first:mt-0 ${size}`}>
            ...
            <ul key={index} className="my-1 space-y-0.5 pl-1">
              {block.items.map((item, i) => (
                <li key={i} className="flex gap-2">
                  <span className="mt-[0.6em] h-1 w-1 shrink-0 rounded-[1px] bg-[var(--strap-text-tertiary)]" />
        ...
        if (block.kind === "numbered") {
          return (
            <ol key={index} className="my-1 space-y-0.5 pl-1">
              {block.items.map((item, i) => (
                <li key={i} className="flex gap-2">
                  <span className="shrink-0 tabular-nums text-[var(--strap-text-tertiary)]">{i + 1}.</span>
                  <span>{renderInline(item, anim)}</span>
                </li>
              ))}
            </ol>
