...
// new version (never destroys history). Read on open; personal Straps don't use
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80vh] overflow-hidden rounded-[var(--radius-xl)] border-[var(--strap-border)] bg-[var(--strap-surface)]">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-10 text-[var(--strap-text-tertiary)]">
            <LoaderCircle className="h-5 w-5 animate-spin" />
          </div>
        ) : versions.length === 0 ? (
          <p className="py-8 text-center text-[14px] text-[var(--strap-text-secondary)]">
            No earlier versions yet.
          </p>
        ) : (
          <div className="max-h-[56vh] space-y-2 overflow-y-auto strap-scrollbar pr-1">
            {versions.map((v, index) => (
              <div
                key={v.id}
                className="flex items-center justify-between gap-4 rounded-[var(--radius-lg)] border border-[var(--strap-border)] px-4 py-3"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-[14px] font-medium text-[var(--strap-text-primary)]">
                    <span>Revision {v.revision}</span>
                    {index === 0 ? (
                      <span className="rounded-md bg-[var(--strap-surface-raised)] px-1.5 py-0.5 text-[11px] font-normal text-[var(--strap-text-tertiary)]">
                        current
                      </span>
                    ) : null}
                  </div>
                  <div className="mt-0.5 truncate text-[12px] text-[var(--strap-text-tertiary)]">
                    {CAUSE_LABEL[v.cause] ?? v.cause}
                    {v.actorType === "agent" && v.agentName
                      ? ` · ${normalizeStrapAttribution(v.agentName)}`
                      : ""}
                ...
                {index === 0 ? null : (
                  <Button
                    variant="outline"
                    className="shrink-0 rounded-md border-[var(--strap-border)]"
