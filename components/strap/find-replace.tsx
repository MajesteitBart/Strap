...
export function StrapFindReplace({
  scrollRef,
}: {
  scrollRef: React.RefObject<HTMLElement | null>;
}) {
  const { state, updateRichTextSection } = useStrap();
    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          className="fixed right-6 top-[76px] z-40 w-[360px] max-w-[calc(100vw-3rem)] rounded-[var(--radius-lg)] bg-[var(--strap-surface)] ring-1 ring-foreground/8 shadow-[0_12px_30px_rgba(28,28,26,0.08)]"
        >
          <div className="flex items-center gap-1 p-1.5">
            <button
              ...
              className="flex h-7 w-5 shrink-0 cursor-grab touch-none items-center justify-center rounded-[6px] text-[var(--strap-text-tertiary)] transition-colors duration-150 hover:text-[var(--strap-text-primary)] active:cursor-grabbing"
            >
              <GripVerticalIcon
                ref={gripRef}
                size={14}
            ...
            <input
              ...
              spellCheck={false}
              autoComplete="off"
              className="h-7 w-full min-w-0 bg-transparent px-1 text-[13px] text-[var(--strap-text-primary)] outline-none placeholder:text-[var(--strap-text-tertiary)]"
            />
            {count ? (
              <span
                className={cn(
          ...
          {replaceOpen ? (
            <div className="flex items-center gap-1 border-t border-[var(--strap-border)] p-1.5 pl-[30px]">
              <input
                value={replacement}
...
/** @deprecated Use StrapFindReplace. Retained for component-import compatibility. */
export const CreedFindReplace = StrapFindReplace;
