...
export function qualityScoreColor(score?: number) {
  if (score === undefined) {
    return "var(--strap-text-tertiary)";
  }

  if (score < 25) {
    return "var(--strap-score-bad)";
  }

  if (score < 75) {
    return "var(--strap-score-mid)";
  }

  return "var(--strap-score-good)";
}

export function qualityToneColor(tone: "good" | "bad" | "neutral") {
  if (tone === "good") {
    return "var(--strap-success)";
  }

  if (tone === "bad") {
    return "var(--strap-danger)";
  }

  return "var(--strap-accent)";
}
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-baseline justify-between gap-3">
        <div
          className="min-w-0 truncate text-[17px] font-medium leading-tight tracking-[-0.01em] text-[var(--strap-text-primary)]"
        ...
        <div className="flex items-baseline gap-1.5">
          ...
          <span className="text-[12px] font-medium text-[var(--strap-text-primary)]">
            / 100
          </span>
        </div>
      </div>
      ...
      {strength || gap ? (
        <div className="space-y-1 border-t border-[var(--strap-border)] pt-2">
...
function QualityNoteRow({ tone, note }: { tone: "good" | "bad"; note: QualityNote }) {
  const [open, setOpen] = useState(false);
  const accent = tone === "good" ? "var(--strap-success)" : "var(--strap-danger)";
  const symbol = tone === "good" ? "+" : "−";
  return (
    <div className="group/note overflow-hidden rounded-md hover:bg-[var(--strap-surface-raised)]">
      <button
        type="button"
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate text-[12px] font-medium text-[var(--strap-text-primary)]">
          {note.title}
        </span>
        <ChevronDown
          className={cn(
            ...
            open
              ? "rotate-180 text-[var(--strap-text-primary)]"
              : "rotate-0 text-[var(--strap-text-tertiary)] group-hover/note:text-[var(--strap-text-primary)]"
          )}
        />
      </button>
      <AnimatePresence initial={false}>
        {open ? (
          ...
          >
            <div className="px-1.5 pb-1.5 pl-[18px] text-[12px] leading-[1.5] text-[var(--strap-text-secondary)]">
              {note.detail}
            </div>
          </motion.div>
        ) : null}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          className="inline-flex h-6 w-6 items-center justify-center rounded-full transition-colors duration-150 hover:bg-[var(--strap-surface-raised)]"
      ...
      <PopoverContent
        ...
        className="relative w-64 rounded-lg bg-[var(--strap-surface)] p-3 shadow-[0_8px_24px_rgba(28,28,26,0.10)] before:pointer-events-auto before:absolute before:-top-2 before:left-0 before:right-0 before:h-2 before:content-['']"
      >
        {quality ? (
          <QualityCompactCard
            score={quality.score}
          ...
          <div className="text-[15px] font-medium leading-none text-[var(--strap-text-secondary)] md:text-[16px]">
            {loading
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        onMouseLeave={isMobile ? undefined : scheduleClose}
        onCloseAutoFocus={(event) => event.preventDefault()}
        className="relative w-72 rounded-lg bg-[var(--strap-surface)] p-3 shadow-[0_8px_24px_rgba(28,28,26,0.10)] before:pointer-events-auto before:absolute before:-top-2 before:left-0 before:right-0 before:h-2 before:content-['']"
      >
        {report ? (
          <QualityCompactCard
          ...
          <div className="text-[15px] font-medium leading-none text-[var(--strap-text-secondary)] md:text-[16px]">
            {loading
              ? "Analyzing your Strap for agent context…"
              : "No analysis yet."}
          </div>
        )}
      </PopoverContent>
