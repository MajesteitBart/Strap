...
import { buildStrapComposePrompt } from "@/lib/strap-prompts";
...
// through them teach what Strap is. The user copies a prompt into any
// assistant, which returns a markdown Strap they paste back. No MCP in
...
export function OnboardingScreen({
  ...
  // state so a resuming, already-composed user still sees their Strap.
  const [composedResult, setComposedResult] = useState<StrapSection[] | null>(null);
  ...
  // A returning user who already has a composed Strap skips onboarding and
  ...
  const handleContinue = useCallback(async () => {
    if (step === EXPLAINER_C_STEP) {
      ...
      // Strap onto, then moves to the prompt step. This is the one loading
  ...
  function handleFinish() {
    // The Strap is persisted server-side (the paste-compose endpoint wrote it),
    // so use a full navigation: /file reloads from server state and renders the
    // composed Strap rather than the provider's possibly-stale seed.
    window.location.href = "/file";
  }

  async function handleCopyPrompt() {
    await navigator.clipboard.writeText(buildStrapComposePrompt(previewSections));
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <StepFrame wide={step >= PROMPT_STEP}>
                    ...
                    <div className="text-center">
                      <AnimatedBlock index={0}>
                        <AnimatedHeadline
                          ...
                          className="t-section justify-center text-[var(--strap-text-primary)]"
                        />
                      </AnimatedBlock>
                      <AnimatedBlock index={1}>
                        <p className="t-lede mx-auto mt-6 text-[var(--strap-text-tertiary)]">
                    ...
                    >
                      <AnimatedBlock index={0}>
                        <Textarea
                          ...
                          className="min-h-[220px] rounded-xl border-[var(--strap-border)] px-4 py-4 text-[15px] leading-7"
                    ...
                    >
                      <AnimatedBlock index={0}>
                        <Textarea
                          ...
                          className="min-h-[200px] rounded-xl border-[var(--strap-border)] px-4 py-4 text-[15px] leading-7"
                  ...
                  {/* Step 9 - Preview the composed Strap before entering the app */}
                  {step === PREVIEW_STEP ? (
                    <div className="text-center">
                      <AnimatedBlock index={0}>
                        <AnimatedHeadline
                      ...
                      <AnimatedBlock index={2}>
                        ...
                        >
                          <StrapPreview sections={composedSections} />
                        </motion.div>
                      </AnimatedBlock>
                    </div>
                  ) : null}
        ...
        <div className="flex items-center justify-between pt-3">
          ...
          {step < TOTAL_STEPS - 1 ? (
            <div className="flex items-center gap-3">
              <span className="hidden text-[12px] text-[var(--strap-text-tertiary)] md:inline">
                ↵ to continue
              </span>
              <Button
...
function OnboardingStep({
  ...
  return (
    <div>
      <AnimatedHeadline
        text={title}
        className="t-section text-[var(--strap-text-primary)]"
      />
...
// The central Strap hub uses Codex's blue (#0066FF) so the one file reads as
...
function ProposalCard() {
  ...
  return (
    <motion.div
      className="mx-auto mt-10 max-w-[520px] rounded-lg border border-[var(--strap-border)] bg-[var(--strap-surface)] text-left shadow-[0_8px_24px_rgba(28,28,26,0.04)]"
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.55, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
    >
      <div className="flex items-center justify-between gap-3 px-3 py-2">
        <div className="flex min-w-0 flex-1 items-center gap-2 text-sm text-[var(--strap-text-secondary)]">
          ...
          <span className="font-medium text-[var(--strap-text-primary)]">Claude</span>
...
// ──────────────────────────────────────────────────────────────────
// Explainer C: the Strap as a plain markdown file the user owns and can
...
function OwnershipCard() {
  return (
    <motion.div
      className="mx-auto mt-10 max-w-[440px] overflow-hidden rounded-lg border border-[var(--strap-border)] bg-[var(--strap-surface)] text-left shadow-[0_8px_24px_rgba(28,28,26,0.04)]"
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.55, delay: 0.1, ease: [0.22, 1, 0.36, 1] }}
    >
      <div className="flex items-center justify-between border-b border-[var(--strap-border)] px-4 py-2.5">
        <div className="flex items-center gap-2 text-[12px] text-[var(--strap-text-secondary)]">
          <FileText className="h-3.5 w-3.5 text-[var(--strap-text-tertiary)]" />
          <span className="font-mono">{STRAP_FILE_NAME}</span>
        </div>
        <div className="flex items-center gap-2 text-[12px] text-[var(--strap-text-secondary)]">
          <Download className="h-3.5 w-3.5 text-[var(--strap-text-tertiary)]" />
      ...
      <div className="px-4 py-4 font-mono text-[13px] leading-6">
        {OWNERSHIP_LINES.map((line, index) => (
          ...
          >
            ...
            <div className="text-[var(--strap-text-secondary)]">{line.body}</div>
          </motion.div>
        ))}
      </div>
    </motion.div>
...
function StrapPreview({ sections }: { sections: StrapSection[] }) {
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] text-left">
