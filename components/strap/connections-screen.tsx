...
} from "@/components/strap/connection-card";
import {
  Dropdown,
  McpHealthDashboard,
} from "@/components/strap/mcp-health-dashboard";
...
import { HeadlessAccessCard } from "@/components/strap/headless-access-card";
...
const CLI_SETUP_STEPS = [
  {
    ...
    detail: "Run Strap directly with npx, with no global install required.",
  },
  {
    title: "Launch it in your terminal",
    detail: "The first run opens Strap's OAuth screen in your browser.",
  },
  {
    title: "Choose your Strap and allow",
      ...
      <div className="mx-auto max-w-[960px] px-4 py-8 md:px-12 md:py-10">
        ...
        <div className="mt-8">
          ...
          <p className="mt-2 text-[14px] leading-7 text-[var(--strap-text-secondary)]">
            {connectionMode === "mcp"
              ? "Paste the server URL into any MCP agent, then authorize Strap in the browser."
              : "Run Strap CLI from any terminal, then authorize the same Strap OAuth screen in your browser."}
          </p>
        </div>
        ...
        <div className="mt-5 grid items-start gap-4 lg:grid-cols-2">
          <div
            className={cn(
              "relative flex h-auto flex-col self-start rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] p-4 transition-opacity duration-160 md:p-5",
              connectionMode === "mcp"
                ? "opacity-100"
          ...
          >
            <button
              ...
              className="absolute inset-0 z-0 cursor-pointer rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[var(--strap-accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--strap-surface)]"
            />
            <div className="pointer-events-none relative z-10 flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <span
                  aria-hidden
                  className="strap-copy-cycle inline-block h-9 w-9 shrink-0"
                  style={{
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-[13px] text-[var(--strap-text-secondary)]">
                    <span
                      className={cn(
                        "h-2 w-2 rounded-[3px]",
                        connected
            ...
            <div className="pointer-events-none relative z-10 mt-4 w-fit max-w-full self-start rounded-[var(--radius-md)] border border-[var(--strap-border)] px-3 py-2 font-mono text-[13px] text-[var(--strap-text-primary)]">
            ...
            <div className="relative z-10 mt-4 flex flex-wrap items-center gap-3">
              ...
              <Button
                variant="ghost"
                className="rounded-md text-[var(--strap-text-secondary)] hover:text-[var(--strap-text-primary)]"
                onClick={() => {
                  setConnectionMode("mcp");
            ...
            <AnimatePresence initial={false}>
              {setupOpen && connectionMode === "mcp" ? (
                ...
                >
                  <ol className="mt-5 grid items-start gap-4 border-t border-[var(--strap-border)] pt-5">
                    {SETUP_STEPS.map((step, index) => (
                      <li key={step.title}>
                        <div className="text-[14px] font-medium text-[var(--strap-text-primary)]">
                          {index + 1}. {step.title}
                        </div>
                        <p className="mt-1 text-[13px] leading-6 text-[var(--strap-text-secondary)]">
                          {step.detail}
                        </p>
                      </li>
                    ))}
          ...
          >
            ...
            <div className="pointer-events-none relative z-10 mt-4 w-fit max-w-full self-start rounded-[var(--radius-md)] border border-[var(--strap-border)] px-3 py-2 font-mono text-[13px] text-[var(--strap-text-primary)]">
              <span className="block break-all">npx @bvdm/strap</span>
            </div>
            ...
            <div className="relative z-10 mt-4 flex flex-wrap items-center gap-3">
              <AnimatedIconButton
                ...
                onClick={() => {
                  ...
                  void copyValue("cli-command", "npx @bvdm/strap");
                }}
              >
                {copied === "cli-command" ? (
                  <>
              ...
              <Button
                variant="ghost"
                className="rounded-md text-[var(--strap-text-secondary)]"
                onClick={() => {
                  setConnectionMode("cli");
                  setSetupOpen((current) => !current);
                }}
        ...
        <div className="mt-10 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-[14px] leading-7 text-[var(--strap-text-secondary)]">
              {connectionMode === "mcp"
                ? "Every agent Strap supports and its MCP connection status."
                : "Use Strap CLI with any agent from the same terminal workflow."}
            </p>
          </div>
          <Dropdown
            trigger={
