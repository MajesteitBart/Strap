...
function accentColor(accent: string | undefined) {
  if (!accent) return "var(--strap-accent)";
...
export function SectionReferencePicker<T extends SectionReferencePickerItem>({
  ...
  return (
    <div
      ...
      {...{ [dataAttribute]: "" }}
      className={cn(
        "fixed z-[60] overflow-y-auto rounded-[var(--radius-md)] bg-[var(--strap-surface)] p-1 ring-1 ring-foreground/8 shadow-[0_12px_30px_rgba(28,28,26,0.08)] strap-scrollbar",
        className,
      )}
      style={{
        pointerEvents: "auto",
    ...
    >
      {items.length ? (
        items.map((section, index) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex h-9 w-full items-center gap-2.5 rounded-[var(--radius-sm)] px-2.5 text-left text-[14px] font-medium text-[var(--strap-text-secondary)] transition-colors duration-150",
        ...
        <div className="flex h-9 items-center px-2.5 text-[13px] text-[var(--strap-text-tertiary)]">
          {emptyMessage}
        </div>
      ) : null}
    </div>
