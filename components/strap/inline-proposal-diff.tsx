...
import { accentColorMap, getProposalPreviewText } from "@/lib/strap-data";
...
export function ProposalAuthor({
  ...
  if (authorType === "user") {
    ...
    return (
      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-[6px] border border-[var(--strap-border)] bg-[var(--strap-surface-raised)] text-[10px] font-medium text-[var(--strap-text-secondary)]">
...
export function InlineProposalDiff({
  ...
  const displayAgentName = normalizeStrapAttribution(agentName);
  ...
  return (
    <div className="rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] shadow-[0_8px_24px_rgba(28,28,26,0.04)]">
      <div className="flex items-center justify-between gap-3 px-3 py-2">
        <button
          type="button"
          onClick={() => setExpanded((v) => !v)}
          className="group/diff flex min-w-0 flex-1 items-center gap-2 text-left text-sm text-[var(--strap-text-secondary)]"
        >
          <ChevronDown
            className={cn(
              // Match the other dropdown chevrons: muted tertiary by default,
              // flips to primary (white in dark) on hover, smooth rotate.
              "h-3.5 w-3.5 shrink-0 text-[var(--strap-text-tertiary)] transition-[transform,colors] duration-200 group-hover/diff:text-[var(--strap-text-primary)]",
        ...
        {mine ? (
          ...
          <div className="-mr-1 flex items-center gap-1">
            ...
            <button
              type="button"
              onClick={onAccept}
              className="inline-flex h-7 items-center gap-1 rounded-md bg-[var(--strap-accent)] px-2.5 text-sm font-medium text-white transition-colors hover:bg-[var(--strap-accent-hover)]"
          ...
          <span className="shrink-0 whitespace-nowrap text-[13px] text-[var(--strap-text-tertiary)]">
            Awaiting review
          </span>
        )}
      </div>
      ...
      <ExpandRegion open={expanded}>
        ...
        {proposal.reason ? (
          <div className="border-t border-[var(--strap-border)] px-4 py-2.5 text-sm leading-5 text-[var(--strap-text-secondary)]">
            {proposal.reason}
          </div>
        ) : null}
      </ExpandRegion>
    ...
    <div className="rounded-xl border border-dashed border-[#10b981]/35 bg-[#ECFDF5]/40 shadow-[0_8px_24px_rgba(16,185,129,0.05)] dark:border-[#22c55e]/35 dark:bg-[#052e1a]/40 dark:shadow-none">
      <div className="flex items-center justify-between gap-3 px-3 py-2">
        <button
          type="button"
          onClick={() => setExpanded((v) => !v)}
          className="flex min-w-0 flex-1 items-center gap-2 text-left text-sm text-[var(--strap-text-secondary)]"
        >
          <ChevronDown
            className={cn(
  ...
  return (
    <div className={containerClass}>
      ...
      <ExpandRegion open={expanded}>
        ...
        <div className="px-4 py-3 text-[14px] leading-7 text-[var(--strap-text-primary)]">
          {isDelete ? (
            <span>
              Removes the entire{" "}
            ...
            <span className="inline-flex items-center gap-2">
              <span className="text-[var(--strap-text-secondary)] line-through">
                {existingName}
              </span>
              <span className="text-[var(--strap-text-tertiary)]">→</span>
            ...
            <span className="inline-flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 text-[var(--strap-text-secondary)] line-through">
                <span
