...
Legacy agent client → capability bearer token → /api/creed/**
                       ...
                       ┌────────────────────────┼───────────────────────┐
                       ▼                        ▼                       ▼
                  Supabase                 OpenRouter            Stripe / GitHub
             auth, Postgres, RLS,       model inference,       payments and file
             realtime, retention        usage and cost         synchronization
```
...
2. Requires either a Personal entitlement or access to a billable Company Creed.
...
6. Mounts `AuthedProviders`, the shell, and `/file`, `/connections`, or `/settings`.
...
- `resolveOwnedCompanyCreedId`: owner-only billing/AI mutation paths.
...
Company writes are per-section, server-authoritative operations in `lib/company-sections.ts` and `app/api/app/sections/**`. The server checks live membership, billing state, role, section permission, and base revision before a service-role write. Realtime broadcasts plus bounded polling reconcile collaborators and agent-originated changes. Section versions support restore.
...
|---|---|---|
| `app/api/app/**` | Supabase session (`auth.getUser`) | Browser product operations |
| `app/mcp/route.ts` | OAuth access bearer token | MCP tools, resources, prompts, and usage attribution |
| `app/api/creed/**` | Hashed capability bearer tokens | Direct HTTP read, proposal, and write integrations |
| OAuth routes | Session during consent; PKCE/code/token afterward | Discovery, registration, authorization, token rotation/revocation |
| Stripe webhook | Stripe raw-body signature | Payment source-of-truth updates |
...
- **Supabase admin clients** perform OAuth, company, webhook, and sensitive integration operations after app-level checks.
- **OpenRouter** receives bounded Creed context and prompts for AI features. Responses are untrusted until parsed and validated.
- **Stripe** is authoritative for paid lifecycle events; webhook handlers provision or freeze access.
...
- Agent behavior: `lib/creed-data.ts`, `app/mcp/route.ts`, `lib/oauth.ts`.
