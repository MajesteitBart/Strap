...
- Follow-up: Windows npm authentication now succeeds as the package maintainer. The failed manual publish targeted the root web package; the tested release artifact is the scoped CLI 0.2.0 tarball. CI caught two brand-audit fingerprints whose line positions moved with the development configuration addition. The compatibility occurrences are unchanged; only those two reviewed fingerprints were refreshed. Dark-mode mobile rendering was also verified through the actual theme control.
