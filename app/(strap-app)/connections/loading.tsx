...
function Block({ className }: { className?: string }) {
  return <div className={`rounded-[6px] bg-[var(--strap-surface-raised)] ${className ?? ""}`} />;
}

function Card({ className, children }: { className?: string; children?: ReactNode }) {
  return (
    <div
      className={`rounded-xl border border-[var(--strap-border)] bg-[var(--strap-surface)] ${className ?? ""}`}
    >
      {children}
    </div>
  );
...
export default function ConnectionsLoading() {
  return (
    <div className="h-full overflow-y-auto bg-[var(--strap-surface)] strap-scrollbar" aria-hidden="true">
          ...
          <div className="mt-12">
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 lg:grid-cols-4">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="rounded-lg border border-[var(--strap-border)] bg-[var(--strap-surface)] p-4"
                >
              ...
              <Card className="p-5">
                ...
                <div className="mt-2 flex items-center gap-6">
                  <div className="flex h-[200px] w-[200px] shrink-0 items-center justify-center">
                    <div className="h-[184px] w-[184px] rounded-full border-[30px] border-[var(--strap-surface-raised)]" />
                  </div>
