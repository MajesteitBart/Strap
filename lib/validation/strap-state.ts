import type { StrapState } from "@/lib/strap-data";
...
export function validateCreedState(input: unknown): CreedStateValidationResult {
  ...
  const arrayKeys: Array<keyof StrapState> = [
    "sections",
    "proposals",
    "activity",
    "connections",
  ...
  return { ok: true, data: input as unknown as StrapState };
}

export type StrapStateValidationResult = CreedStateValidationResult;
export const validateStrapState = validateCreedState;
