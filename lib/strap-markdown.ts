import { isAccentKey, type AccentKey, type StrapSection } from "./strap-data.ts";
...
// (see buildVisibleStrapMarkdown). Read it back here so recolored sections
...
function parseSectionBody(
  ...
  function makeRichTextSection(content: string, template: StrapSection["template"] = "freeform"): StrapSection {
    return {
      id,
      kind: "rich-text",
      template,
...
export function parseStrapMarkdown(markdown: string): ParseResult {
...
/** @deprecated Use parseStrapMarkdown. */
export const parseCreedMarkdown = parseStrapMarkdown;
