...
// per-section permissions, rename, ownership transfer, and delete. All run
...
export async function setMemberRole(params: {
  ...
  const { error } = await query(
    ...
    "update",
    async (database, scope) => {
      const values = { role: params.role } as Partial<
...
export async function removeMember(params: {
  ...
  await query(
    db,
    tables.creed_member_section_permissions,
    "delete",
    (database, scope) =>
      database
        .delete(tables.creed_member_section_permissions)
        .where(
          and(
            scope,
            eq(
              tables.creed_member_section_permissions.creed_id,
              params.creedId,
            ),
            eq(
              tables.creed_member_section_permissions.user_id,
              params.targetUserId,
            ),
          ),
        ),
  ...
  const { data: tokens } = (await query(
    ...
    (database, scope) =>
      database
        .select({ id: tables.oauth_tokens.id })
        .from(tables.oauth_tokens)
        .where(
          and(scope, eq(tables.oauth_tokens.user_id, params.targetUserId)),
        ),
  )) as {
    data: Array<{ id: string }> | null;
  };
  if (tokens && tokens.length > 0) {
    await query(db, tables.oauth_token_creeds, "delete", (database, scope) =>
      database.delete(tables.oauth_token_creeds).where(
        and(
          scope,
          eq(tables.oauth_token_creeds.creed_id, params.creedId),
          inArray(
            tables.oauth_token_creeds.token_id,
            tokens.map((t) => t.id),
          ),
        ),
      ),
...
export async function setSectionPermission(params: {
  ...
  const { error } = await query(
    ...
    async (database, scope) => {
      ...
      return database
        .insert(tables.creed_member_section_permissions)
        .values(values)
        .onConflictDoUpdate({
          target: [
            tables.creed_member_section_permissions.creed_id,
            tables.creed_member_section_permissions.user_id,
            tables.creed_member_section_permissions.section_id,
          ],
          set: conflictSet(tables.creed_member_section_permissions, values),
          setWhere: scope,
        });
    },
  );
...
export async function updateCompanyGeneral(params: {
  ...
  const { error } = await query(
    ...
    async (database, scope) => {
      ...
      return database
        .update(tables.creeds)
        .set(values)
        .where(and(scope, eq(tables.creeds.id, params.creedId)));
    },
  );
  if (error) {
    return {
      ok: false,
      error: "Could not update company settings.",
      status: 500,
    };
  }
  return { ok: true };
...
export async function transferOwnership(params: {
  ...
  const { error: transferError } = await callProcedure(
    rpc,
    "transfer_creed_ownership",
    {
...
export async function deleteCompany(params: {
  ...
  const blocker = await checkLegacyDeletion(db, {
    scope: "company",
    strapId: params.creedId,
  });
  ...
  const { error } = await query(
    ...
    (database, scope) =>
      database
        .delete(tables.creeds)
        .where(and(scope, eq(tables.creeds.id, params.creedId))),
  );
  if (error) {
    return {
      ok: false,
      error: "Could not delete the company Strap.",
      status: 500,
    };
  }
  return { ok: true };
