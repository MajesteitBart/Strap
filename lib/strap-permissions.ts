...
// A Strap is a single-user personal file or a shared company file.
export type StrapType = "personal" | "company";

// The three V1 roles. Exactly one owner per Strap (enforced in the schema).
export type StrapRole = "owner" | "admin" | "member";

/** @deprecated Use StrapType. */
export type CreedType = StrapType;
/** @deprecated Use StrapRole. */
export type CreedRole = StrapRole;
...
// The mode a user picks for one of their MCP agents on one Strap. Spelled
...
export function canManageMembers(role: StrapRole): boolean {
  return role === "owner" || role === "admin";
}

/** Owner/Admin: archive, delete, and reorder sections. */
export function canManageSectionsLifecycle(role: StrapRole): boolean {
  return role === "owner" || role === "admin";
}

/** Owner/Admin: run shared company quality analysis. */
export function canRunAnalysis(role: StrapRole): boolean {
  return role === "owner" || role === "admin";
}
